...
public class HBaseShellCommands {
    ...
    public static Set<String> getAllCommands() throws IOException {
        String commandsPath = String.format("%s/hbase-ruby/shell/commands",
            ...
            synchronized (HBaseShellCommands.class) {
                if (commandsSet == null) {
                    ...
                    if (commandFilesUrl == null) {
                        throw new IOException(String.format("The commands file path %s is not exists",
                                commandsPath));
                    }
                    String commandFilePath = commandFilesUrl.getPath();
                    File commandFile = new File(commandFilePath);
                    if (!commandFile.exists()) {
                        LOG.warn(String.format("The commands file path %s is not exists, starting read file from jar",
                                commandFilePath));
