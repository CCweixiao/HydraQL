...
import com.hydraql.exceptions.HBaseShellSessionEnvInitException;
...
public class HBaseShellSession implements ShellSession {
      ...
      try {
        ...
        for (int i = 0; i < initMaxTimes; i++) {
          try {
            ...
            if (i == (initMaxTimes - 1)) {
              LOGGER.error("After {} retries, HBase shell session initialization failed.",
                initMaxTimes, ex);
  ...
  private void createShellRunningEnv() throws IOException {
    ...
    String commandsFilePath =
        String.format("%s/hbase-ruby/hirb.rb", HBaseShellConstants.HBASE_ADAPTER_VERSION);
  ...
  public Result execute(String cmd) {
    if (!this.isSessionConnected()) {
      return Result.failed(
        String.format("The current session [%s] is not connected," + " please try again.", this));
    }
