...
/**
 * Abstract class to adapt the default ycsb DB interface to Timeseries databases. This class is
 * mostly here to be extended by Timeseries dataabases originally developed by Andreas Bader in
 * <a href="https://github.com/TSDBBench/YCSB-TS">YCSB-TS</a>.
 * <p>
 * This class is mostly parsing the workload information passed through the default ycsb interface
 * according to the information outlined in {@link TimeSeriesWorkload}. It also contains some minor
 * utility methods relevant to Timeseries databases.
 * </p>
 * @implSpec It's vital to call <tt>super.init()</tt> when overwriting the init method to correctly
 *           initialize the workload-parsing.
 */
public abstract class TimeseriesDB extends DB {
  ...
  public void init() throws DBException {
    ...
    deleteDelimiter = getProperties().getProperty(TimeSeriesWorkload.DELETE_DELIMITER_PROPERTY,
      TimeSeriesWorkload.DELETE_DELIMITER_PROPERTY_DEFAULT);
    timestampUnit =
        TimeUnit.valueOf(getProperties().getProperty(TimeSeriesWorkload.TIMESTAMP_UNITS_PROPERTY,
          TimeSeriesWorkload.TIMESTAMP_UNITS_PROPERTY_DEFAULT));
    groupByKey = getProperties().getProperty(TimeSeriesWorkload.GROUPBY_KEY_PROPERTY,
      TimeSeriesWorkload.GROUPBY_KEY_PROPERTY_DEFAULT);
    downsamplingKey = getProperties().getProperty(TimeSeriesWorkload.DOWNSAMPLING_KEY_PROPERTY,
      TimeSeriesWorkload.DOWNSAMPLING_KEY_PROPERTY_DEFAULT);
    downsamplingFunction = AggregationOperation.valueOf(getProperties().getProperty(
      TimeSeriesWorkload.DOWNSAMPLING_FUNCTION_PROPERTY, DOWNSAMPLING_FUNCTION_PROPERTY_DEFAULT));
    downsamplingInterval = Integer.valueOf(getProperties().getProperty(
      TimeSeriesWorkload.DOWNSAMPLING_INTERVAL_PROPERTY, DOWNSAMPLING_INTERVAL_PROPERTY_DEFAULT));

    test = Boolean.parseBoolean(getProperties().getProperty(TEST_PROPERTY, TEST_PROPERTY_DEFAULT));
    debug =
        Boolean.parseBoolean(getProperties().getProperty(DEBUG_PROPERTY, DEBUG_PROPERTY_DEFAULT));
  }

  @Override
  public final Status read(String table, String key, Set<String> fields,
  ...
  /**
   * @inheritDoc
   * @implNote this method parses the information passed to it and subsequently passes it to the
   *           modified interface at
   *           {@link #scan(String, long, long, Map, AggregationOperation, int, TimeUnit)}
   */
  @Override
  public final Status scan(String table, String startkey, int recordcount, Set<String> fields,
    ...
    for (String field : fields) {
      if (field.startsWith(timestampKey)) {
        ...
        if (!downsamplingSpec
            .equals(downsamplingFunction.toString() + downsamplingInterval.toString())) {
          System.err
              .print("Downsampling specification for Scan did not match configured downsampling");
          return Status.BAD_REQUEST;
        }
      } else {
        String[] queryParts = field.split(tagPairDelimiter);
    ...
    return scan(table, start, end, tagQueries, downsamplingFunction, downsamplingInterval,
      timestampUnit);
  }

  /**
   * Perform a range scan for a set of records in the database. Each value from the result will be
   * stored in a HashMap.
   * @param metric The name of the metric
   ...
   */
  protected abstract Status scan(String metric, long startTs, long endTs,
      Map<String, List<String>> tags, AggregationOperation aggreg, int timeValue,
      TimeUnit timeUnit);
  ...
  /**
   * Insert a record into the database. Any tags/tagvalue pairs in the specified tagmap and the
   * given value will be written into the record with the specified timestamp.
   * @param metric The name of the metric
   ...
   */
  protected abstract Status insert(String metric, long timestamp, long value,
      Map<String, ByteIterator> tags);

  /**
   * Insert a record in the database. Any tags/tagvalue pairs in the specified tagmap and the given
   * value will be written into the record with the specified timestamp.
   * @param metric The name of the metric
   * @param timestamp The timestamp of the record to insert.
   ...
   * @return A {@link Status} detailing the outcome of the insert
   */
  protected abstract Status insert(String metric, long timestamp, double value,
      Map<String, ByteIterator> tags);

  /**
   * NOTE: This operation is usually <b>not</b> supported for Time-Series databases. Deletion of
   * data is often instead regulated through automatic cleanup and "retention policies" or similar.
   * @return Status.NOT_IMPLEMENTED or a {@link Status} specifying the outcome of deletion in case
  ...
  /**
   * Examines the given {@link Properties} and returns an array containing the Tag Keys (basically
   * matching column names for traditional Relational DBs) that are detailed in the workload
   * specification. See {@link TimeSeriesWorkload} for how these are generated.
   * <p>
   ...
   * @return An array of strings specifying all allowed TagKeys (or column names) except for the
   *         "value" and the "timestamp" column name.
   * @implSpec WARNING this method must exactly match how tagKeys are generated by the
   *           {@link TimeSeriesWorkload}, otherwise databases requiring this information will most
   *           likely break!
   */
  protected static String[] getPossibleTagKeys(Properties properties) {
    final int tagCount =
        Integer.parseInt(properties.getProperty(TimeSeriesWorkload.TAG_COUNT_PROPERTY,
          TimeSeriesWorkload.TAG_COUNT_PROPERTY_DEFAULT));
    final int tagKeylength =
        Integer.parseInt(properties.getProperty(TimeSeriesWorkload.TAG_KEY_LENGTH_PROPERTY,
  ...
  /**
   * An enum containing the possible aggregation operations. Not all of these operations are
   * required to be supported by implementing classes.
   * <p>
   * Aggregations are applied when using the <tt>SCAN</tt> operation on a range of timestamps. That
   * way the result set is reduced from multiple records into a single one or one record for each
   * group specified through <tt>GROUP BY</tt> clauses.
   */
  public enum AggregationOperation {
    ...
    NONE,
    /**
     * Sum the values of the matching records when calculating the value. GroupBy criteria apply
     * where relevant for sub-summing.
     */
    SUM,
    /**
     * Calculate the arithmetic mean over the value across matching records when calculating the
     * value. GroupBy criteria apply where relevant for group-targeted averages
     */
    AVERAGE,
    /**
     * Count the number of matching records and return that as value. GroupBy criteria apply where
     * relevant.
     */
    COUNT,
    /**
     * Return only the maximum of the matching record values. GroupBy criteria apply and result in
     * group-based maxima.
     */
    MAX,
    /**
     * Return only the minimum of the matching record values. GroupBy criteria apply and result in
     * group-based minima.
     */
    MIN;
  }
