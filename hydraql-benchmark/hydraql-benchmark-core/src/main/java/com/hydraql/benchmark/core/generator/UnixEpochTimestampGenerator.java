...
/**
 * A generator that produces Unix epoch timestamps in seconds, milli, micro or nanoseconds and
 * increments the stamp a given interval each time {@link #nextValue()} is called. The result is
 * emitted as a long in the same way calls to {@code System.currentTimeMillis()} and
 * {@code System.nanoTime()} behave.
 * <p>
 * By default, the current system time of the host is used as the starting timestamp. Calling
 * {@link #initalizeTimestamp(long)} can adjust the timestamp back or forward in time. For example,
 * if a workload will generate an hour of data at 1 minute intervals, then to set the start
 * timestamp an hour in the past from the current run, use:
 * 
 * <pre>
 * {
 *   &#64;code
 ...
 * </pre>
 * 
 * A constructor is also present for setting an explicit start time. Negative intervals are
 * supported as well for iterating back in time.
 * <p>
 * WARNING: This generator is not thread safe and should not called from multiple threads.
 */
public class UnixEpochTimestampGenerator extends Generator<Long> {
   ...
   *          millis, {@code System.currentTimeMillis()} or seconds and any interval above,
  ...
  /**
   * Sets the starting timestamp to the current system time plus the interval offset. E.g. to set
   * the time an hour in the past, supply a value of {@code -60}.
  ...
  public void initalizeTimestamp(final long intervalOffset) {
    switch (timeUnits) {
      ...
      case MILLISECONDS:
        ...
        break;
      case SECONDS:
        currentTimestamp = (System.currentTimeMillis() / 1000) + getOffset(intervalOffset);
        break;
      case MINUTES:
        currentTimestamp = (System.currentTimeMillis() / 1000) + getOffset(intervalOffset);
        break;
