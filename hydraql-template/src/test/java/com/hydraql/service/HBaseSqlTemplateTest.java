...
public class HBaseSqlTemplateTest extends AbstractHBaseTemplateTest {
  ...
  public void testInsertNullAndEmptyStr() {
    String hql = String.format(
      "upsert into test:test_sql ( row_key , f1:id , f1:name , f1:age , f1:job , f1:pay , f2:address , f2:commuter ) "
          + "values ('%s', '%s' , '%s' , null , '%s' , %.2f , '北京' , '%s' )",
      "10001", "10001", "leojie", "程序员", 20000 * new Random().nextInt(10) * 0.1 / 3 + 10000, "");
  ...
  public void testCreateVirtualTable() {
    ...
    String tableDesc =
        sqlTemplate.showCreateVirtualTable("show create virtual table test:test_sql;");
  ...
  public void testSelectInRows() {
    String sql3 = "select * from" + " test:test_sql where " + "rowKey " + "in('a1000','a1002')";
  ...
  public void testSelectTimeRange() {
    String hql = "select * from test:test_sql where " + "rowKey = '10001'"
  ...
  public void testSelectFilterOri() throws IOException {
    ...
    FilterList payOrJobFilter =
        new FilterList(FilterList.Operator.MUST_PASS_ONE, Arrays.asList(payFilter, jobFilter));
    ...
    for (String commuter : commuters) {
      filters.add(new SingleColumnValueFilter(Bytes.toBytes("f2"), Bytes.toBytes("commuter"),
          CompareFilter.CompareOp.NOT_EQUAL, Bytes.toBytes(commuter)));
    }
    ...
    for (Result result : scanner) {
      ...
      System.out.printf(
        "row_key: %s\tid: %s\tname: %s\tage: %s\tjob: %s\tpay: %s\taddress: %s\tcommuter: %s%n",
  ...
  public void testSelectMaxVersions() {
    String hql =
        "select f1:id from test:test_sql where rowkey = '10011' versions 5 (startTs>=1695564244983, endTs <= 1695564245041)";
    HBaseDataSet dataSet = sqlTemplate.select(hql);
    dataSet.show();
  }
  ...
  public void testSelect() {
    ...
    String sql4 =
        "select * from test:test_sql where ( startKey = 'a1000' , endKey = 'j1009' ) and ( f1:age >= 25 and ( f1:job IS NOT NULL ) ) ";
    HBaseDataSet dataSet4 = sqlTemplate.select(sql4);
    dataSet4.show();
    System.out.println("============================================================");

    String sql5 =
        "select * from test:test_sql where ( startKey = 'a1000' , endKey = 'j1009' ) and ( f1:age >= 25 and ( f1:job IS NULL or f1:pay > 15000 ) )";
    ...
    //
    // System.out.println("============================================================");
    // String sql32 = "select f1:name , f1:age from test:test_sql where rowKey = 'row_1000' and
    // maxVersion = 3";
  ...
  public void testSelectMaxVersion2() {
    String hsql1 =
        "insert into test:test_sql ( f1:id ) values ( '11111_v1' ) where rowKey = 'row_10002'";
    String hsql2 =
        "insert into test:test_sql ( f1:id , f1:age ) values ( '11111_v2' , 14 ) where rowKey = 'row_10002'";
    sqlTemplate.insert(hsql1);
    sqlTemplate.insert(hsql2);
  ...
  public void testDeleteByStartAndEnd() {
    String hql = "delete from  test:test_sql where startkey > 'a1000'" + " and endkey <= 'a1002'";
    sqlTemplate.delete(hql);
  }
  ...
  public String defaultTableSchemaJsonFormat() {
    return "{" + "\t\"tableName\": \"test_table\"," + "\t\"defaultFamily\": \"cf\","
        + "\t\"columnList\": [{" + "\t    \"familyName\": \"\","
        + "\t    \"columnName\": \"rowKey\"," + "\t    \"columnType\": \"string\","
        + "\t    \"columnIsRow\": \"true\"" + "\t},{" + "\t    \"familyName\": \"cf\","
