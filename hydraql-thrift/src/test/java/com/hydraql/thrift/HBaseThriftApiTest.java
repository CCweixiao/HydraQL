...
public class HBaseThriftApiTest extends BaseHBaseThriftTemplateTest {
  ...
  public void testGetRowsToPOJO() {
    try (HBaseThrift hBaseThrift = thriftPool.getResource()) {
      GetRowsParam getRowsParam1 =
          GetRowsParam.of(Arrays.asList("a11001", "a12002")).family("info").build();

      List<CityModel> rows = hBaseThrift.getRows(getRowsParam1, CityModel.class);

      Assert.assertEquals(1, rows.size());
      GetRowsParam getRowsParam2 = GetRowsParam.of(Arrays.asList("a11001", "a12002")).family("info")
  ...
  public void testScanToPOJO() {
    ScanParams params = ScanParams.of().familyName("").limit(2).reversed(true).build();
    try (HBaseThrift hBaseThrift = thriftPool.getResource()) {
  ...
  public void testScanToMap() {
    ScanParams params = ScanParams.of().familyName("").limit(10).reversed(true).build();
