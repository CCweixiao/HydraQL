...
public final class UnsafeAccess {
  ...
  /**
   * This number limits the number of bytes to copy per call to Unsafe's copyMemory method. A limit
   * is imposed to allow for safepoint polling during a large copy
   */
  ...
  public static int putByte(ByteBuffer buf, int offset, byte b) {
    if (buf.isDirect()) {
      ...
      THE_UNSAFE.putByte(buf.array(), BYTE_ARRAY_BASE_OFFSET + buf.arrayOffset() + offset, b);
    }
    return offset + 1;
  }
