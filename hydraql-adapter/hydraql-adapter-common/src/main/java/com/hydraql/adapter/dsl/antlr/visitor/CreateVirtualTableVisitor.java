...
public class CreateVirtualTableVisitor extends BaseVisitor<HBaseTableSchema> {
  ...
  public HBaseTableSchema
      visitCreate_virtual_table_command(HydraQLParser.Create_virtual_table_commandContext ctx) {
      ...
      if (primaryKeyNode != null) {
        if (StringUtil.isNotBlank(familyName)) {
          throw new HBaseSqlAnalysisException(
              "Illegal rowKey naming format " + columnDefContext.column_ref().getText());
        }
    ...
    if (tableOptionsContext != null) {
      ...
      for (HydraQLParser.OptionContext optionContext : optionContextList) {
        ...
        switch (optionName) {
          ...
          default:
            throw new HBaseSqlAnalysisException(
                String.format("Unsupported table property option [%s].", optionName));
        }
      }
    }
  ...
  private HBaseColumn getRow(String filedName, String fieldType, boolean nullAble) {
    ...
    return HBaseColumn.of("", filedName).columnIsRow(true)
        .columnType(ColumnType.getColumnType(fieldType)).nullable(false).build();
  }

  private HBaseColumn getColumn(String familyName, String columnName, String fieldType,
      boolean nullAble) {
    return HBaseColumn.of(familyName, columnName).nullable(nullAble)
  ...
  private String extractColType(HydraQLParser.Data_typeContext dataTypeContext) {
    // todo 处理字段类型长度
    // HydraQLParser.Precision_intContext precisionIntContext =
    // dataTypeContext.sql_data_type().precision_int();
  ...
  public HBaseTableSchema
      extractHBaseTableSchema(HydraQLParser.Create_virtual_table_commandContext ctx) {
