...
public abstract class BaseHTableDesc {
  ...
  /**
   * the maximum size of the store file after which a region split occurs.
   */
  ...
  /**
   * the maximum size of the memstore after which its contents are flushed onto the disk
   */
  ...
  public abstract static class Builder<HTD extends BaseHTableDesc> implements ConfigFilter {
      ...
      for (BaseColumnFamilyDesc familyDesc : this.columnFamilyDescList) {
        if (name.equals(familyDesc.getName())) {
          throw new IllegalArgumentException(
              String.format("Column family %s already exists", name));
        }
      }
    ...
    public Builder<HTD> copyFrom(HTD htd) {
      setMaxFileSize(htd.getMaxFileSize()).setMemStoreFlushSize(htd.getMemStoreFlushSize())
          .setReadOnly(htd.getReadOnly()).setDurability(htd.getDurability())
          .setPriority(htd.getPriority()).setCompactionEnabled(htd.getCompactionEnabled())
          .setNormalizationEnabled(htd.getNormalizationEnabled())
          .setMergeEnabled(htd.getMergeEnabled()).setSplitEnabled(htd.getSplitEnabled())
  ...
  public void appendColumnFamily(BaseColumnFamilyDesc columnFamilyDesc) {
    ...
    for (BaseColumnFamilyDesc cf : this.getColumnFamilyDescList()) {
      if (cf.getName().equals(columnFamilyDesc.getName())) {
        throw new IllegalArgumentException(
            String.format("Column family %s already exists", columnFamilyDesc.getName()));
      }
    }
    this.columnFamilyDescList.add(columnFamilyDesc);
  }
