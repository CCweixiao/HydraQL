...
public class HBaseAdminAdapter extends AbstractHBaseAdminAdapter implements HBaseMetricOperations {
  ...
  public List<HTableDesc> listTableDesc(String regex, boolean includeSysTables) {
    return this.execute(admin -> {
      ...
      for (HTableDescriptor tableDescriptor : tableDescriptors) {
        HTableDesc tableDesc = HTableDesc.createDefault(tableDescriptor.getNameAsString())
            .convertFrom(tableDescriptor);
  ...
  public List<HTableDesc> listTableDescByNamespace(String namespaceName) {
    return this.execute(admin -> {
      final HTableDescriptor[] tableDescriptors =
          admin.listTableDescriptorsByNamespace(namespaceName);
      if (tableDescriptors == null || tableDescriptors.length == 0) {
        return new ArrayList<>();
      }
      return Arrays.stream(tableDescriptors)
          .map(t -> HTableDesc.createDefault(t.getNameAsString()).convertFrom(t))
  ...
  public List<ColumnFamilyDesc> listFamilyDescOfTable(String tableName) {
    return this.execute(admin -> {
      ...
      return families.stream()
          .map(c -> ColumnFamilyDesc.createDefault(c.getNameAsString()).convertFrom(c))
          .collect(Collectors.toList());
    });
  }
  ...
  public HTableDesc getTableDesc(String tableName) {
    return this.execute(admin -> {
      HTableDescriptor tableDescriptor = admin.getTableDescriptor(TableName.valueOf(tableName));
      return HTableDesc.createDefault(tableDescriptor.getNameAsString())
          .convertFrom(tableDescriptor);
    });
  }
  ...
  public List<String> listTableNamesByNamespace(String namespaceName) {
    return listTableDescByNamespace(namespaceName).stream().map(HTableDesc::getName)
        .collect(Collectors.toList());
  }
  ...
  public <HTD extends BaseHTableDesc> boolean createTable(HTD tableDesc) {
    ...
    return this.execute(admin -> {
      HTableDescriptor tableDescriptor = ((HTableDesc) tableDesc).convertTo();
  ...
  @Override
  public <HTD extends BaseHTableDesc> boolean createTable(HTD tableDesc, String startKey,
      String endKey, int numRegions, boolean isAsync) {
    Assert.checkNotNull(tableDesc);
    return this.execute(admin -> {
      HTableDescriptor tableDescriptor = ((HTableDesc) tableDesc).convertTo();
      tableExistsThrowError(admin, tableDescriptor.getTableName().getNameAsString());
      boolean preSplit =
          (StringUtil.isNotBlank(startKey) && StringUtil.isNotBlank(endKey) && numRegions > 0);
  ...
  @Override
  public <HTD extends BaseHTableDesc> boolean createTable(HTD tableDesc, String[] splitKeys,
      boolean isAsync) {
    Assert.checkNotNull(tableDesc);
    return this.execute(admin -> {
      HTableDescriptor tableDescriptor = ((HTableDesc) tableDesc).convertTo();
  ...
  @Override
  public <HTD extends BaseHTableDesc> boolean createTable(HTD tableDesc, SplitGoEnum splitGoEnum,
      int numRegions, boolean isAsync) {
    Assert.checkNotNull(tableDesc);
    return this.execute(admin -> {
      HTableDescriptor tableDescriptor = ((HTableDesc) tableDesc).convertTo();
      tableExistsThrowError(admin, tableDescriptor.getTableName().getNameAsString());
  ...
  @Override
  public boolean modifyTableAttributes(String tableName, Map<String, String> attributes,
      boolean isAsync) {
    ...
    return this.execute(admin -> {
      ...
      for (String attributeKey : attributes.keySet()) {
        ...
        if (StringUtil.isBlank(attributeValue)) {
          ...
          ImmutableBytesWritable originalValue =
              values.get(new ImmutableBytesWritable(Bytes.toBytes(attributeKey)));
  ...
  @Override
  public boolean modifyTableConfiguration(final String tableName, Map<String, String> configuration,
      boolean isAsync) {
  ...
  public boolean modifyFamilyAttributes(String tableName, String familyName,
    ...
    return this.execute(admin -> {
      ...
      for (String attributeKey : attributes.keySet()) {
        ...
        if (StringUtil.isNotBlank(originalAttributeVal) && StringUtil.isNotBlank(modifyAttributeVal)
            && originalAttributeVal.equals(modifyAttributeVal)) {
          continue;
        }
  ...
  public boolean modifyFamilyConfiguration(String tableName, String familyName,
    ...
    return this.execute(admin -> {
      ...
      for (String configKey : configs.keySet()) {
        ...
        if (StringUtil.isNotBlank(originalConfigVal) && StringUtil.isNotBlank(modifyConfigVal)
            && originalConfigVal.equals(modifyConfigVal)) {
          continue;
        }
  ...
  public boolean renameTable(String oldTableName, String newTableName, boolean deleteOldTable,
    ...
    return this.execute(admin -> {
      ...
      final String tempSnapShotName = oldTableName.replace(":", "_") + "_SNAPSHOT_RENAME_"
  ...
  @Override
  public <CF extends BaseColumnFamilyDesc> boolean addFamily(String tableName, CF familyDesc,
      boolean isAsync) {
    ...
    return this.execute(admin -> {
      ...
      if (tableDescriptor.hasFamily(Bytes.toBytes(columnFamilyDesc.getName()))) {
        throw new HBaseFamilyHasExistsException(String.format(
          "The family %s in the table %s has created.", columnFamilyDesc.getName(), tableName));
      }
      admin.addColumn(TableName.valueOf(tableName), columnFamilyDesc.convertTo());
      return true;
    });
  }
  ...
  public boolean deleteFamily(String tableName, String familyName, boolean isAsync) {
    return this.execute(admin -> {
      ...
      if (!tableDescriptor.hasFamily(Bytes.toBytes(familyName))) {
        throw new HBaseFamilyNotFoundException(
            String.format("The family %s in the table %s is not exists.", familyName, tableName));
      }
  ...
  @Override
  public <CF extends BaseColumnFamilyDesc> boolean modifyFamily(String tableName, CF familyDesc,
      boolean isAsync) {
    Assert.checkNotNull(familyDesc);
    return this.execute(admin -> {
      tableNotExistsThrowError(admin, tableName);
      ...
      final HTableDescriptor tableDescriptor =
          admin.getTableDescriptor(TableName.valueOf(tableName));
      if (!tableDescriptor.hasFamily(Bytes.toBytes(columnFamilyDesc.getName()))) {
        throw new HBaseFamilyNotFoundException(String.format(
          "The family %s in the table %s is not exists.", columnFamilyDesc.getName(), tableName));
      }
      HColumnDescriptor oldColumnDescriptor =
          tableDescriptor.getFamily(Bytes.toBytes(columnFamilyDesc.getName()));
      HColumnDescriptor newColumnDescriptor = columnFamilyDesc.convertTo();
  ...
  @Override
  public <CF extends BaseColumnFamilyDesc> boolean modifyFamilyAsync(String tableName,
      CF familyDesc) {
    return false;
  }
  ...
  public boolean createNamespace(NamespaceDesc namespaceDesc, boolean isAsync) {
    ...
    return this.execute(admin -> {
      NamespaceDescriptor namespaceDescriptor =
          NamespaceDescriptor.create(namespaceDesc.getNamespaceName()).build();
  ...
  public boolean snapshot(SnapshotDesc snapshotDesc, boolean isAsync) {
    return this.execute(admin -> {
      ...
      admin.snapshot(snapshotDesc.getSnapshotName(),
  ...
  public boolean mergeTableSmallRegions(String tableName, int limitRegionsNum,
    ...
    return this.execute(admin -> {
      ...
      clusterStatus.getServers().forEach((serverName -> clusterStatus.getLoad(serverName)
          .getRegionsLoad().forEach((region, regionLoad) -> {
      ...
      for (int i = 0, j = 1; i < subMergedRegions.size() - 1; i += 2, j += 2) {
        ...
        admin.mergeRegions(firstRegionInfo.getEncodedNameAsBytes(),
          secondRegionInfo.getEncodedNameAsBytes(), false);
      }
      return true;
    });
  }
   ...
   */
  private boolean modifyTableReplicationScope(String tableName, int replicationScope,
  ...
  public Summary refreshSummary() {
    return this.execute(admin -> {
      ...
      Summary clusterSummary = new Summary(currentTime, version, clusterId,
          liveServers + deadServers, liveServers, deadServers, namespaceCount, tableCount,
          snapshotCount, regionCount, ritCount, averageLoad, aggregateRequestPerSecond);
      List<String> liveServerNames = clusterStatus.getServers().stream()
          .map(ServerName::getServerName).collect(Collectors.toList());
      List<String> deadServerNames = clusterStatus.getDeadServerNames().stream()
          .map(ServerName::getServerName).collect(Collectors.toList());
  ...
  public List<HBaseTableRecord> refreshTableRecords(Field currentSortField, boolean ascendingSort) {
    return this.execute(admin -> {
      ...
      return records.stream().map(this::convertRecordToHBaseTableRecord)
          .collect(Collectors.toList());
    });
  }
  ...
  private HBaseTableRecord convertRecordToHBaseTableRecord(Record record) {
    ...
    tableRecord
        .setStoreFileSize(record.get(Field.STORE_FILE_SIZE).asSize().get(Size.Unit.GIGABYTE));
    tableRecord
        .setUncompressedStoreFileSizeTag(record.get(Field.UNCOMPRESSED_STORE_FILE_SIZE).asString());
    tableRecord.setUncompressedStoreFileSize(
      record.get(Field.UNCOMPRESSED_STORE_FILE_SIZE).asSize().get(Size.Unit.GIGABYTE));
    ...
    return this.execute(admin -> {
      ...
      for (int i = 0, j = 1; i < records.size(); i += 1, j += 1) {
        ...
        if (j > records.size() - 1) {
          ...
          endKey = secondRegionRecord
              .getOrDefault(Field.START_KEY, new FieldValue("", FieldValueType.STRING)).asString();
        }
        ...
        regionRecord.setStartCode(DateAndTimeUtil.parseTimestampToTimeStr(
          Long.parseLong(firstRegionRecord.get(Field.START_CODE).asString())));
        ...
        regionRecord.setStoreFileSize(
          firstRegionRecord.get(Field.STORE_FILE_SIZE).asSize().get(Size.Unit.GIGABYTE));
        regionRecord.setUncompressedStoreFileSizeTag(
          firstRegionRecord.get(Field.UNCOMPRESSED_STORE_FILE_SIZE).asString());
        regionRecord.setUncompressedStoreFileSize(firstRegionRecord
            .get(Field.UNCOMPRESSED_STORE_FILE_SIZE).asSize().get(Size.Unit.GIGABYTE));
        ...
        regionRecord.setMemStoreSize(
          firstRegionRecord.get(Field.MEM_STORE_SIZE).asSize().get(Size.Unit.MEGABYTE));
  ...
  private List<Record> filterAndSortRecords(List<Record> records, List<RecordFilter> recordFilters,
    ...
    if (currentSortField != null) {
      sortAndFilterRecords =
          records.stream().filter(r -> recordFilters.stream().allMatch(f -> f.execute(r)))
