...
public class HqlExecutor extends BaseExecutor {
  ...
  @Override
  public <E> Put buildPut(E entity, Class<E> entityClass) {
    if (null == entity) {
      return null;
    }
  ...
  @Override
  public <E> List<Put> buildPuts(List<E> entities, Class<E> entityClass) {
    if (null == entities) {
  ...
  private <E> Put buildPut(E entity) {
    HFieldInfo.RowKey rowKey = this.getTableInfo().getRowKey();
    List<HFieldInfo.Qualifier> qualifiers = this.getTableInfo().getQualifiers();
