...
public final class RegionServerModeStrategy implements ModeStrategy {
  ...
  public List<Record> getRecords(ClusterMetrics clusterMetrics) {
    ...
    List<Record> records = regionModeStrategy.getRecords(clusterMetrics).stream().map(
    ...
    Map<String, Record> retMap = records.stream()
        .collect(Collectors.groupingBy(r -> r.get(Field.LONG_REGION_SERVER).asString())).entrySet()
        .stream()
        .flatMap(
    ...
    for (ServerMetrics sm : clusterMetrics.getLiveServerMetrics().values()) {
      ...
      Record newRecord =
          Record.builder().putAll(record).put(Field.USED_HEAP_SIZE, sm.getUsedHeapSize())
