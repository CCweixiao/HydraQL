...
public abstract class BaseVisitor<T> extends HydraQLParserBaseVisitor<T> {
  ...
  private Object extractConstantVal(HBaseColumn column,
      HydraQLParser.ConstantContext constantContext) {
  ...
  protected List<Object> extractConstantValList(
      HydraQLParser.ConditionValListContext conditionValListContext, HBaseColumn column,
  ...
  public QueryExtInfo parseQueryExtInfo(HydraQLParser.Select_commandContext selectCommandContext) {
    ...
    HydraQLParser.Versions_clauseContext versionsClauseContext =
        selectCommandContext.versions_clause();
    ...
    if (timestampRangeClauseContext != null) {
      TimeStampRange timeStampRange =
          extractTimeStampRange(this.getTableSchema(), timestampRangeClauseContext);
  ...
  public long extractTimeStamp(HydraQLParser.TsExpContext tsExpContext) {
    ...
    String error = String.format("The timestamp %s is not a standard timestamp format, "
        + "please enter a 13-bit Unix timestamp.",
      ts);
    if (ts.length() != 13) {
