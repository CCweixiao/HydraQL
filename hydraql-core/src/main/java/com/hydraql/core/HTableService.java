...
public interface HTableService {
   ...
   * </p>
   * <p>
   * 如果对数据类型有严格要求，请使用方法: {@link HTableService#save(Object)} <br/>
   * <p/>
   ...
   * } <br/>
   * <h3>tip:</h3> 1. HBase表中数据都是以字节形成存储，存取时都需要字段值的类型，才能对值做正确的类型转换，否则会出现乱码字符，导致读取数据解析数据异常。
   * 因此在此API的封装中，非字符串的基础类型数据，底层都会被自动序列化成字符串类型。<br/>
   * 2. 字段名称的命名格式必须是：family:qualifier <br/>
   * 3. 复杂数据类型，如：List/Map/Java对象等，会先被格式化成JSON字符串后再进行存储，默认使用FastJson框架来序列化/反序列化json数据<br/>
   * 4. rowKey只能是字符串类型
   * <p>
   * 如果对数据类型有要求，请使用方法: {@link HTableService#saveBatch(List<Object>)} <br/>
   * <p/>
   * @param tableName 表名
   ...
   * 数据实体类的定义参考：例如：{@link com.hydraql.common.model.example.CityModel} <br/>
   * @param t 数据实体对象
   */
  <T> void save(T t);
   ...
   * {<br/>
   * &nbsp;&nbsp;&nbsp;&nbsp; rowKey: "row1", &nbsp;&nbsp;&nbsp;&nbsp; "f1:name": {"value": "leo",
   * "timestamp": 1667568422619},<br/>
   * &nbsp;&nbsp;&nbsp;&nbsp; "f1:age": {"value": "18", "timestamp": 1667568422619} <br/>
   * }
   * @param tableName 表名
   ...
   * @return 查询结果
   */
  <T> List<T> getWithMultiVersions(String tableName, GetRowParam getRowParam,
   ...
   * &nbsp;&nbsp;&nbsp;&nbsp;"rowKey": "1001", <br/>
   * &nbsp;&nbsp;&nbsp;&nbsp;{<br/>
   * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"f1:id": [{"value": "i2", timestamp: 2}, {"value": "i1",
   * timestamp: 1}],<br/>
   * &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"f2:age": [{"value": 17, timestamp: 2}, {"value": 18,
   * timestamp: 1}]<br/>
   * &nbsp;&nbsp;&nbsp;&nbsp;}<br/>
   * &nbsp;&nbsp;}<br/>
   * }<br/>
  ...
  /**
   * 构造{@link com.hydraql.common.query.ScanParams}条件来扫描多条数据，默认只返回最新版本的数据，<br/>
   * 并把查询结果Result的字段和值，转换为{@link com.hydraql.common.model.data.HBaseRowData}列表
   * @param tableName 表名
   * @param scanParams scan查询参数
   * @return 查询结果集
   */
  ...
  /**
   * 构造{@link com.hydraql.common.query.ScanParams}条件来扫描多条数据，默认只返回最新版本的数据，<br/>
   * 并把查询结果Result的字段和值，转换为{@link com.hydraql.common.model.data.HBaseRowDataWithMultiVersions}列表，
   * @param tableName 表名
   * @param scanParams scan查询参数
   * @return 查询结果集
   */
  List<HBaseRowDataWithMultiVersions> scanWithMultiVersions(String tableName,
